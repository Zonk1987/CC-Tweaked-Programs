...
- **交互式设置** — 无需编辑代码。该脚本在第一次运行时询问目标位置。
- **配置菜单与命令行选项** — 在计算机终端上按“C”，或使用 `--config` / `-c` 参数运行脚本，即可更改目标名称或频道。
...
4. **配置**：如果您将计算机移动到新的基地，可以随时更改目标名称或频道：
   - **热键**：在程序运行期间，在键盘上按“C”打开配置菜单。
   - **命令行**：使用 `--config` 或 `-c` 参数运行程序：
     ```bash
     startup.lua --config
     ```
     或：
