...
local function getIngredientSummary(recipe)
    ...
    table.sort(summary, function(a, b) return a.char < b.char end)
    return summary
end
...
function ManageRecipes:open()
    ...
    while true do
        ...
        for i = 1, maxLines do
            ...
            local name = r.name
            if #name > listWidth - 3 then name = name:sub(1, listWidth - 5) .. ".." end
            term.write(name)
        end
        ...
        if current then
            ...
            for i, item in ipairs(summary) do
                ...
                if #shortName > detailW - 5 then shortName = shortName:sub(1, detailW - 7) .. ".." end
        ...
        if side == "right" then footer = "F:Fuzzy | " .. footer end
        term.write(footer)

        local _, key = os_pullEvent("key")
        if key == keys_up then
            if side == "left" and selected > 1 then selected = selected - 1
            elseif side == "right" and rightSelected > 1 then rightSelected = rightSelected - 1 end
        elseif key == keys_down then
            if side == "left" and selected < #self.recipeManager.recipes then selected = selected + 1
            elseif side == "right" and rightSelected < #summary then rightSelected = rightSelected + 1 end
