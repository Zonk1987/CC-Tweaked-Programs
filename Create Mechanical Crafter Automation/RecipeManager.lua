...
function RecipeManager:generateExample(crafterCount)
    ...
    for _ = 1, padTop do table_insert(pattern, emptyRow) end
    for _, row in ipairs(basePattern) do
        table_insert(pattern, padLeftStr .. row .. padRightStr)
    end
    for _ = 1, padBottom do table_insert(pattern, emptyRow) end
...
function RecipeManager:findReadyRecipe(chest, crafterCount)
    ...
    for _, recipe in ipairs(self.recipes) do
        if self:isRecipeComplete(recipe, chestItems, crafterCount) then return recipe end
    end
    return nil
end
...
function RecipeManager:getMissingItems(chest, crafterCount)
    ...
    for _, recipe in ipairs(self.recipes) do
        ...
        for k, v in pairs(recipe.ingredients) do
            local numK = tonumber(k)
            if numK and numK > maxIdx then maxIdx = numK end
            if type(v) == "string" and v ~= "null" and v ~= "" then
...
function RecipeManager:recordRecipe(name, crafterGrid)
    ...
    if width ~= math.floor(width) then return false, "Grid not square!" end
    ...
    for i = 1, count do
        ...
        if item then
            ...
            if reverseKeys[item.name] then char = reverseKeys[item.name]
            else
        ...
        if i % width == 0 then table_insert(pattern, currentLine) currentLine = "" end
    end
